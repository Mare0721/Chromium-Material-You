...
void NavigatorLanguage::EnsureUpdatedLanguage() {
  ...
  if (config && config->ua.enabled && !config->ua.language.empty()) {
    languages_ = ParseAndSanitize(config->ua.language);
    languages_dirty_ = false;  // Config is static, no need to mark dirty
    return;
  }
