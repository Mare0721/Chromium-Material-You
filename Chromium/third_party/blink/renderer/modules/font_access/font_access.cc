...
ScriptPromise<IDLSequence<FontMetadata>> FontAccess::QueryLocalFontsImpl(
  ...
  if (FingerprintConfig::IsFontNoiseEnabled()) {
    auto* resolver =
        MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<FontMetadata>>>(
            script_state, exception_state.GetContext());
    auto promise = resolver->Promise();

    HeapVector<Member<FontMetadata>> spoofed_fonts;

    // [核心修改] 从配置中心获取 JSON 加载的动态列表
    const Vector<String>& safe_fonts =
        FingerprintConfig::Instance().GetFontWhitelist();

    // 如果 JSON 为空，给一个最基础的保底
    if (safe_fonts.empty()) {
      auto entry = FontEnumerationEntry{.postscript_name = "Arial",
                                        .full_name = "Arial",
                                        .family = "Arial",
                                        .style = "Regular"};
      spoofed_fonts.push_back(FontMetadata::Create(std::move(entry)));
    } else {
      for (const String& name : safe_fonts) {
        // 保持原有的 options 过滤逻辑
        if (options->hasPostscriptNames()) {
          bool found = false;
          for (const auto& req : options->postscriptNames()) {
            if (req == name) {
              found = true;
              break;
            }
    ...
    resolver->Resolve(std::move(spoofed_fonts));
    return promise;
  }
  // ================= [FINGERPRINT MOD END] =================
