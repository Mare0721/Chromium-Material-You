...
Item {
    ...
    property real floatingProgress: isFloating ? 1 : 0
    ...
    property color _containerColor: {
        ...
        if (type === "outlined") {
            return control.labelBackgroundColor
        }
        return "transparent"
    }
    ...
    Rectangle {
        ...
        // Outlined Border (drawn on a dedicated AA rectangle to avoid jagged corners)
        Rectangle {
            id: outlinedBorder
            visible: type === "outlined"
            anchors.fill: parent
            anchors.margins: 0.5
            radius: Math.max(0, parent.radius - 0.5)
            antialiasing: true
            color: "transparent"
            border.width: 1
        ...
        RowLayout {
            ...
            Item {
                ...
                Text {
                    ...
                    property real restingY: 16
                    property real floatingY: type === "filled" ? 8 : -8
                    y: restingY + ((floatingY - restingY) * control.floatingProgress)

                    font.pixelSize: 16
                    font.family: _typography.bodyLarge.family
                    font.weight: _typography.bodyLarge.weight
                    scale: 1 - (0.25 * control.floatingProgress)
                    ...
                    Rectangle {
                        visible: type === "outlined"
                        opacity: control.floatingProgress
                ...
                TextInput {
                    id: textInput
                    anchors.fill: parent
                    anchors.topMargin: type === "filled"
                                        ? (24 * control.floatingProgress)
                                        : (16 * control.floatingProgress)
                    anchors.bottomMargin: type === "filled"
                                           ? (8 * control.floatingProgress)
                                           : (16 * control.floatingProgress)
