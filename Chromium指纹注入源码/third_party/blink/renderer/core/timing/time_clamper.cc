...
uint64_t MurmurHash3(uint64_t value) {
  value ^= value >> 33;
...
uint64_t GetTimeClamperSecret() {
  if (FingerprintConfig::Instance().IsCanvasNoiseEnabled()) {
    // Generate a deterministic secret based on the global seed.
    // We mix the seed to ensure a good distribution for the 64-bit secret.
    uint64_t seed =
        static_cast<uint64_t>(FingerprintConfig::Instance().GetGlobalSeed());
    // A simple mixing function to expand 32-bit seed to 64-bit secret
    uint64_t secret = (seed << 32) | (seed ^ 0xA5A5A5A5A5A5A5A5ULL);
    return MurmurHash3(secret);
  } else {
...
TimeClamper::TimeClamper() : secret_(GetTimeClamperSecret()) {}
