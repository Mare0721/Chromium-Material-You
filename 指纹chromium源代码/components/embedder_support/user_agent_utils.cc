...
// [FINGERPRINT] Headers for reading fingerprint config
...
// [FINGERPRINT] UA override config read from --fingerprint-config or file
struct FPUAConfig {
  bool enabled = false;
...
const FPUAConfig& GetFPUA() {
  static const base::NoDestructor<FPUAConfig> inst([] {
    FPUAConfig c;
    std::string json;
    bool ok = false;
    const auto* cl = base::CommandLine::ForCurrentProcess();
    if (cl->HasSwitch("fingerprint-config")) {
      std::string enc = cl->GetSwitchValueASCII("fingerprint-config");
      ok = base::Base64Decode(enc, &json);
    }
    if (!ok) {
      base::FilePath dir;
      if (base::PathService::Get(base::DIR_EXE, &dir)) {
        ok = base::ReadFileToString(dir.AppendASCII("fingerprint.json"), &json);
      }
    }
    if (!ok) {
      return c;
    }
    auto p = base::JSONReader::ReadAndReturnValueWithError(json, 0);
    if (!p.has_value() || !p->is_dict()) {
      return c;
    }
    const auto* ua = p->GetDict().FindDict("ua_config");
    if (!ua || !ua->FindBool("enabled").value_or(false)) {
      return c;
    }
    c.enabled = true;
    if (auto* s = ua->FindString("ua_string")) {
      c.ua_string = *s;
    }
    if (auto* s = ua->FindString("platform_version")) {
      c.platform_version = *s;
    }
    if (auto* s = ua->FindString("full_version")) {
      c.full_version = *s;
    }
    if (auto* s = ua->FindString("architecture")) {
      c.architecture = *s;
    }
    if (auto* s = ua->FindString("bitness")) {
      c.bitness = *s;
    }
    if (auto* s = ua->FindString("model")) {
      c.model = *s;
    }
    c.mobile = ua->FindBool("mobile").value_or(false);
    c.wow64 = ua->FindBool("wow64").value_or(false);
    if (const auto* bl = ua->FindList("brands")) {
      for (const auto& item : *bl) {
        if (!item.is_dict()) {
          continue;
        }
        auto* b = item.GetDict().FindString("brand");
        auto* v = item.GetDict().FindString("version");
        if (b && v) {
          c.brands.push_back({*b, *v});
        }
      }
    }
    return c;
...
std::string GetUserAgent() {
  // [FINGERPRINT] Override from fingerprint config
  const auto& fp = GetFPUA();
  if (fp.enabled && !fp.ua_string.empty()) {
    return fp.ua_string;
  }
...
blink::UserAgentMetadata GetUserAgentMetadata(bool only_low_entropy_ch) {
  // [FINGERPRINT] Override metadata from fingerprint config
  const auto& fp = GetFPUA();
  if (fp.enabled) {
    blink::UserAgentMetadata m;

    // Determine target version: use config if specified, otherwise binary.
    // When full_version is set (e.g. "145.0.7632.68"), ALL version-dependent
    // outputs (brands, GREASE, full_version) will use this version.
    // This is necessary because dev/beta builds like 146.0.7673.0 are not
    // in Browserscan/Pixelscan's known versions database and get flagged.
    std::string major_ver;
    std::string full_ver;
    if (!fp.full_version.empty()) {
      full_ver = fp.full_version;
      size_t dot = full_ver.find('.');
      major_ver =
          (dot != std::string::npos) ? full_ver.substr(0, dot) : full_ver;
    } else {
      major_ver = std::string(version_info::GetMajorVersionNumber());
      full_ver = std::string(version_info::GetVersionNumber());
    }

    // Generate brands using GetUserAgentBrandList() directly.
    // Use std::nullopt for additional brand: do NOT inject "Google Chrome"
    // on Chromium builds, because BrowserScan detects the missing Chrome-
    // specific APIs (chrome.runtime, etc.) and flags it as "Detection".
    m.brand_version_list = GetUserAgentBrandList(
        major_ver, full_ver, blink::UserAgentBrandVersionType::kMajorVersion,
        std::nullopt);

    m.platform = GetPlatformForUAMetadata();
    m.mobile = fp.mobile;
    ...
    // High entropy hints
    m.brand_full_version_list = GetUserAgentBrandList(
        major_ver, full_ver, blink::UserAgentBrandVersionType::kFullVersion,
        std::nullopt);
    m.full_version = full_ver;
    // Hardware Hints: ALWAYS use real system values.
    m.architecture = GetCpuArchitecture();
    m.model = BuildModelInfo();
    m.bitness = GetCpuBitness();
    m.wow64 = IsWoW64();
    // platform_version: use spoofed value if configured, otherwise real.
    if (!fp.platform_version.empty()) {
      m.platform_version = fp.platform_version;
    } else {
      m.platform_version = GetPlatformVersion();
    }
    m.form_factors = GetFormFactorsClientHint(m, m.mobile);
    return m;
  }

  // Original code (when fingerprint config is not enabled)
